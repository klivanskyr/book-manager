...
export default function ProfileSettings({ params }: { params: { userId: string }}) {
    ...
    return (
        <div className='w-auto h-full p-16 m-8 shadow-large flex flex-col'>
            ...
            <div className='flex flex-row'>
                <div className='py-2 pr-4 pl-1'>
                    <div className="flex flex-row items-center">
                        <h2 className="mr-4 w-[100px]">Profile Image</h2>
                        <Skeleton className={`w-[500px] my-2 rounded-2xl ${isLoaded ? 'opacity-100' : 'opacity-40'}`} isLoaded={isLoaded}>
                            <Input classNames={{ "mainWrapper": "w-[500px] my-2 font-semibold" }} placeholder='Image url' size='lg' variant='bordered' value={input.image} onChange={(e) => setInput({ ...input, image: e.target.value })}/>
                        </Skeleton>
                        <Button className='mx-1 h-[45px]' color='primary' onClick={() => handleUpdate({ newImage: input.image})}>Upload</Button>
                    </div>
                    <div className="flex flex-row items-center">
                        <h2 className="mr-4 w-[100px]">Username</h2>
                        <Skeleton className={`w-[500px] my-2 rounded-2xl ${isLoaded ? 'opacity-100' : 'opacity-40'}`} isLoaded={isLoaded}>
                            <Input classNames={{ "mainWrapper": "w-[500px] my-2 font-semibold" }} size='lg' variant='bordered' value={input.username} onChange={(e) => setInput({ ...input, username: e.target.value })}/>
                        </Skeleton>
                    ...
                    <div className="flex flex-row items-center">
                        <h2 className="mr-4 w-[100px]">Email</h2>
                        <Skeleton className={`w-[500px] my-2 rounded-2xl ${isLoaded ? 'opacity-100' : 'opacity-40'}`} isLoaded={isLoaded}>
                            <Input classNames={{ "mainWrapper": "w-[500px] my-2 font-semibold" }} size='lg' variant='bordered' value={input.email} onChange={(e) => setInput({ ...input, email: e.target.value })}/>
                        </Skeleton>
                    ...
                    <div>
                        <p className={`text-center ${message.includes('Error') || message.includes('error') ? 'text-red-600' : 'text-blue-600'}`}>
                            {message}
                        </p>
                    </div>
                ...
                {/* Photos */}
                <div className='flex flex-row justify-center items-center w-[600px] h-[350px] mx-16 text-center'>
                    <div className='shadow-medium p-2 m-2 rounded-lg'>
                        <h3 className='font-light'>Current Profile Image</h3>
                        <Image src={userData?.image} alt="Current Profile Image" height={150} width={150} />
                    </div>
                    <div className='shadow-medium p-2 m-2 rounded-lg'>
                        <h3 className="font-light">New Profile Image</h3>
                        <Image src={input.image} alt="New Profile Image" height={150} width={150} />
                    </div>
                </div>
            </div>
        </div>
